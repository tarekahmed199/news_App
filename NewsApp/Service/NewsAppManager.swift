...
class NewsApiManager {
    ...
    func getNewsFeed(category : category , word : String? = "",completion: @escaping ([article]?,Error?) -> Void) {
        
        guard  let url : URL = newsApiURL.getNewsFeed(searchWord: word ?? "", category: category).url else {
            return
        }
